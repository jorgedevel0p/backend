...
def producto_api_view(request):
  ...
  if request.method == 'GET':
    ...
    productos_serializer = ProductoSerializer(productos, many = True) # (many = True) ---> List of productos, it expects only 1 object
    return Response(productos_serializer.data, status.HTTP_200_OK)

  # create resource
  if request.method == 'POST':
    producto_serializer = ProductoSerializer(data = request.data) # get json and compare to model
